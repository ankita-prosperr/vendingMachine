...
function Inventory() {
  const { id } = useParams(); // vendingMachineId
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8080/admin/vending-machines/${id}/slots`)
      .then(res => res.json())
      .then(data => {
        setSlots(data);
        setLoading(false);
  ...
  const handleCartChange = (slot, change) => {
    setCartItems(prev => {
      const newCart = { ...prev };
      const currentCount = newCart[slot.slotId]?.count || 0;
      const newCount = currentCount + change;

      if (newCount <= 0) {
        delete newCart[slot.slotId];
      } else {
        newCart[slot.slotId] = { slot, count: newCount };
      }
    ...
    <div className="p-6 relative">
      ...
      <div className="flex flex-wrap gap-6 justify-start">
        {slots.map(slot => (
          <ItemCard
            key={slot.slotId}
            item={slot}                       // slot acts as item
            count={cartItems[slot.slotId]?.count || 0}
            onCartChange={(change) =>
              handleCartChange(slot, change)
            }
          />
        ))}
      </div>
